  ...
  <div>
    ...
    <el-card>
      <el-tabs v-model="activeName">
        <el-tab-pane label="编辑教师基本信息" name="first">
          <el-card shadow="hover" style="width: 700px;" v-loading="loading">
            ...
            <el-form :model="ruleForm" ref="ruleForm" label-width="100px">
              ...
              <el-form-item label="登录密码" prop="password">
                <el-input type="password" v-model="ruleForm.password" placeholder="可不填,默认密码不做修改"></el-input>
              </el-form-item>
...
import { requestGetTeacher, requestEditTeacher, requestImgLimit } from '@/api/teacher'
...
export default {
  ...
  methods: {
    ...
    getTeacher() {
      this.loading = true
      // 查询教师信息，先填写在表单里
      requestGetTeacher(this.$route.params.id).then(response => {
        if (response.success) { // 找到了这个人
          let result = response.result
          this.ruleForm = result
          this.ruleForm.works = JSON.parse(result.works)
          this.ruleForm.roles = result.roles.split(',')
          this.loading = false
        } else { // 没查询到
          this.$router.push('/admin/teacher/list') // 跳转回列表页
          this.$message({
    ...
    submitForm(formName) {
      this.$refs[formName].validate(valid => {
        ...
        if (valid) {
          ...
          requestEditTeacher(this.ruleForm).then(result => {
            this.loading = false
            // 提交成功
            this.$message({
