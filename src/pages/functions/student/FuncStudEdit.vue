  ...
  <div>
    <el-card>
      <el-tabs v-model="activeName">
        <el-tab-pane label="编辑学员基本信息" name="first">
...
import { requestGetStudent, requestEditStudent } from '@/api/student'
export default {
  name: 'FuncStudEdit',
  created() {
    this.setImgLimit()
  ...
  methods: {
    ...
    getStudent() {
      this.loading = true
      // 查询学生信息，先填写在表单里
      requestGetStudent(this.$route.params.id).then(response => {
        if (response.success) { // 找到了这个人
          ...
          this.$router.push('/admin/student/list') // 跳转回列表页
          this.$message({
    ...
    submitForm(formName) {
      this.$refs[formName].validate(valid => {
        ...
        if (valid) {
          ...
          requestEditStudent(this.ruleForm).then(result => {
            this.loading = false
            // 提交成功
            this.$message({
